 ...
 */
auto mk_struct_field_matcher(string_t const & sname){
...
public:
  ...
  using func_mem_map_t = std::map<string_t,set_t>; // Key: func name, Vals: Members
  ...
  matchers_t matchers() const {
    ...
    for (auto t : targets_)
    {
      ms.push_back(mk_struct_field_matcher(t));
    }
    return ms;
  }
  ...
  void run(const result_t & result) override {
    ...
    if(membr && func)
    {
      ...
      if(on_lhs)
      {
        lhs_uses_[s_name][f_name].insert(m_name);
      }
      else
      {
