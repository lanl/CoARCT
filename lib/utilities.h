...
namespace corct {
inline string_t add_tab(string_t const s) {
  return s + '\t';
}
...
inline vec_str split(std::string const& s, char const delim) {
...
/**\brief Complain if pointer is invalid.
  ...
  \return none */
template <typename T>
inline void check_ptr(T* p, string_t const name, string_t const tabs = "") {
  if (!p) {
...
*/
inline string_t get_struct_name(clang::MemberExpr const& membr) {
...
inline string_t infer_indent_level(clang::SourceLocation& loc,
...
/**\brief True if node is on the LHS of operator=
  ...
  operations.
  */
inline bool is_on_lhs(clang::MemberExpr const* membr, clang::ASTContext& ctx) {
...
template <
    // typename NodeT1,
    typename NodeT2>
inline bool is_ancestor_of(clang::BinaryOperator const* e1,
...
inline bool is_part_of_assignment(clang::MemberExpr const* membr,
                                  clang::ASTContext& ctx) {
  using namespace clang;
  string_t const s_name = get_struct_name(*membr);
  string_t const m_name = membr->getMemberDecl()->getNameAsString();
...
inline bool compare_vars(clang::VarDecl const* const v1,
