...
Replacement
gen_new_call(CallExpr * call_expr,
             ...
             bool const verbose)
{
  ...
  if(!has_defaults) {
    ...
    ait--;
    Expr * last(*ait);
    if(verbose) { corct::dumpSourceRange(last->getSourceRange(), &sm); }
    ...
    uint32_t n(0);
    for(p_iter pit = func_decl->param_begin(); pit != func_decl->param_end();
        ++pit, ++ait) {
...
bool
param_matches(ParmVarDecl * p_decl,
              ...
              bool const verbose = false)
{
  ...
  bool matches = parm_name == p_name && type_name == t_name;
  if(!matches && verbose) {
...
Replacement
gen_signature_repl(FunctionDecl * f_decl,
                   ...
                   bool const verbose)
{
    ...
    if(param_matches(*pit, type_name, old_name)) {
      ...
      if(verbose) {
        std::cout << "gen_signature_repl: parameter with name '"
                  << (*pit)->getNameAsString() << "' matches " << old_name
                  << std::endl;
      }
    }
    else {
  ...
  Replacement repl;
  if(nullptr != p_decl) {
    repl = replace_source_range(sm, p_decl->getSourceRange(), new_param);
  }
  return repl;
}
...
Replacement
gen_new_signature(FunctionDecl * f_decl,
                  ...
                  SourceManager const & sm)
{
  ...
  // arguments. Not sure that's always true. If not, the else
  // branch below may be borken.
  // TODO Is there a better way to determine if a function has default
  // arguments?
  bool const has_defaults =
