...
replace_source_range(clang::SourceManager const & sm,
...
inline replacement_t
replace_source_range_naive(clang::SourceManager const & sm,
                           clang::SourceRange const & range,
                           clang::StringRef const replacement)
{
  using clang::SourceLocation;
  ...
  SourceLocation start_end_loc =
      range.getEnd();  // Start of last token at end of range
  unsigned int length =
      sm.getFileOffset(start_end_loc) - sm.getFileOffset(start_loc);
  return replacement_t(sm, start_loc, length, replacement);
}

/** Insert replacement string after the source range. */
inline replacement_t
append_source_range(clang::SourceManager const & sm,
                    clang::SourceRange const & range,
                    clang::StringRef const replacement)
{
  using clang::SourceLocation;
  ...
  SourceLocation end_loc(clang::Lexer::getLocForEndOfToken(
      start_end_loc, 0, sm, lopt));  // True end of range
  // unsigned int length = sm.getFileOffset(end_loc) -
  // sm.getFileOffset(start_loc) ;
  return replacement_t(sm, end_loc, 0, replacement);
}  // append_source_range

/** Insert replacement string after the source range. */
inline replacement_t
append_source_loc(clang::SourceManager const & sm,
                  clang::SourceLocation const & start_end_loc,
                  clang::StringRef const replacement)
{
  using clang::SourceLocation;
  // SourceLocation start_loc = range.getBegin();     // Start of range
  // SourceLocation start_end_loc = range.getEnd();   // Start of last token at
  // end of range
  clang::LangOptions lopt;
  SourceLocation end_loc(clang::Lexer::getLocForEndOfToken(
      start_end_loc, 0, sm, lopt));  // True end of range
...
/** \brief insert a parameter if it't the only one in the function.
   ...
   */
inline replacement_t
insert_only_param(clang::SourceManager const & sm,
                  ...
                  clang::StringRef const replacement)
{
  ...
  clang::LangOptions lopt;
  SourceLocation end_loc(clang::Lexer::getLocForEndOfToken(
      start, 0, sm, lopt));  // True end of range
...
inline replacement_t
prepend_source_range(clang::SourceManager const & sm,
                     clang::SourceRange const & range,
                     clang::StringRef const replacement)
{
  using clang::SourceLocation;
  SourceLocation start_loc = range.getBegin();  // Start of range
  // SourceLocation start_end_loc = range.getEnd();   // Start of last token at
  // end of range clang::LangOptions lopt; SourceLocation end_loc(
  //   clang::Lexer::getLocForEndOfToken(start_end_loc, 0, sm, lopt));  // True
  //   end of range
  // unsigned int length = sm.getFileOffset(end_loc) -
  // sm.getFileOffset(start_loc) ;
...
/** Insert replacement string before the source range (only uses the start). */
inline replacement_t
prepend_source_loc(clang::SourceManager const & sm,
